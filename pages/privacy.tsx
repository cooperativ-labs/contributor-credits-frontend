...
export type PrivacyText = string;

const Contracts: React.FC<ResultProps> = () => {
  const data = `/assets/privacy-policy.md`;
  const getPrivacyText = async (): Promise<PrivacyText> => axios.get(data).then((resp) => resp.data);
  const { value: privacyText } = useAsync(getPrivacyText, []);

  return (
    <div data-test="component-landing" className="bg-white flex flex-col w-full h-full">
      <Head>
        <title>Cooperativ - Privacy Policy</title>
        <meta name="viewport" content="initial-scale=1.0, width=device-width" />
        <meta property="og:title" content="Cooperativ - Privacy Policy" />
        ...
        <meta property="og:url" content="https://cooperativ.io/privacy" />
        {/** Twitter */}
        <meta name="twitter:title" content="Cooperativ - Privacy Policy" />
      ...
      <div className="md:py-10 pb-10 px-8">
        <div className="flex-col min-h-full mx-auto pt-10 prose" style={{ maxWidth: '1000px' }}>
          <ReactMarkdown source={privacyText} />
        </div>
      </div>
      <Footer />
    </div>
